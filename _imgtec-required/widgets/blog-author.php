...
class Blog_Author extends WP_Widget {
   ...
   */
	public function widget( $args, $instance ) {
		...
		echo '<p>Search using a choice of ' . $total . ' authors.</p>';		
		echo '<form>';
		echo '<select onChange="window.location.replace(this.options[this.selectedIndex].value)">';
		echo '<option value="">Select an Author</option>';		
		foreach ( $authors as $author ){
      		echo '<option value="'. get_author_posts_url( $author->ID, $author->user_nicename ) .'">' . esc_html( $author->display_name ) .' (' . count_user_posts($author->ID) .')</option>';
		}
		echo '</select>';
