...
add_filter( 'manage_press_releases_posts_columns', 'press_releases_columns' );
...
function press_releases_manage_columns( $column ){
	...
	switch ( $column ) {
		...
		case 'type':
			$release_types = get_the_terms( $post->ID, 'release_types' );
			...
			if ( !empty( $release_types ) ) {
				...
				foreach ($release_types as $type) {
					$types_out[] = sprintf(
						'<a href="%s">%s</a>',
						esc_url( add_query_arg( array( 'post_type' => $post->post_type, 'release_types' => $type->slug ), 'edit.php' ) ),
						esc_html( sanitize_term_field( 'name', $type->name, $type->term_id, 'release_types', 'display' ) )
					);
				}
				echo join( ', ', $types_out );
			}
...
add_action( 'manage_press_releases_posts_custom_column', 'press_releases_manage_columns', 10, 2 );
