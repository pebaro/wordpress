...
function imgtec_subscribe_to_mailing_list(){ ?>
    <section id="imgtec-subscribe-to-mailing-list">
        <h1>Stay up-to-date with Imagination</h1>
        <p>Sign up and receive the latest news and product updates from Imagination straight to your inbox.</p>
        <form action="" method="post">
            <p><input type="email" class="form-control imgtec-mailing-subscribe" name="email" id="email" placeholder="Enter your email address" required></p>
            <input type="hidden" name="news-letter-success" value="submitted">
            <p><input type="submit" class="btn-subscribe" value="Subscribe"></p>
        </form>
        <p><a href="http://www2.imgtec.com/preferences" target="_blank">Manage your subscription or unsubscribe</a></p>
    </section>
    <?php
}
...
function imgtec_masonry_list_terms_links( $cpt_names ){
	...
	foreach ( $all_cpt_taxonomies as $cpt_taxonomy ) {
		...
		foreach ( $tax_terms as $tax_term ) {
			...
			$terms_list[] = '<a href="' . $terms_href . '" class="imgtec-tax-term">' . $tax_term->name . '</a>';
		}
		// format terms list
...
function imgtec_list_all_taxonomies_with_selected_terms(){
	...
	foreach ($all_taxonomies as $taxonomy) {
		...
		foreach ( $all_terms as $term ) {
			...
			$terms_list[] = '<a href="/' . $href . '" class="imgtec-tax-term">' . $term->name . '</a>';
		}
		// format the list
		$output_terms = join( ', ', $terms_list );

		// strip out underscores and hyphens
		$taxonomy_output = str_replace( '-', ' ', $taxonomy->name );
		...
		$combined_output .= ucwords( $taxonomy_output ) . ':</strong> ';
...
function taxonomy_slug_rewrite( $wp_rewrite ){
    ...
    // loop through the post types and taxonomies
    foreach ( $custom_post_types as $cpt ) {
        foreach ( $custom_taxonomies as $ctax ) {

            // go through the post types this taxonomy is assigned to
            foreach ( $ctax->object_type as $obj_type ) {

                // check if this cpt has this taxonomy
